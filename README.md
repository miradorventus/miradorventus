...
It produces numbers that are consistent, plausible and wrong. Two examples from
my own benchmarks: disabling a model's reasoning channel to get a readable
answer produced a clean, monotonic quality curve that measured the flag rather
than the model; and a context window overrun does not truncate gracefully on
llama.cpp, it silently sacrifices exactly half the context. Both looked fine.

**Check what exists before building it.** Half of a twenty-script measurement
suite I wrote already existed elsewhere, better. Reading the repository beat
reading the search results every single time.
